...
What happens if you try to access `spam['foo']` if `spam` is `{'bar': 100}`? | `KeyError`<br>Unless you a) set a default value<br>b) try: `spam['foo']` except `KeyError`: whatever else you want to do<br>c) if 'foo' in spam: ...
